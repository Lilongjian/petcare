<%@page import="qdu.petcare.pojo.*"%>
<%@page import="java.util.*"%>
<%@page import="qdu.petcare.vo.PageBean"%>
		...
		<div class="row">
			<div class="col-md-12">
					...
					<table
						...
						<c:forEach items="${pageBean.list}" var="orders">
							<tr class="warning">
								...
								<td>${orders.status}</td>
								<td><fmt:formatDate value="${orders.paymentTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><fmt:formatDate value="${orders.sendTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><fmt:formatDate value="${orders.endTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><fmt:formatDate value="${orders.closeTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><fmt:formatDate value="${orders.createTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><fmt:formatDate value="${orders.updateTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
				...
				<ul class="pagination">
					...
					<c:if test="${pageBean.pageIndex!=1}">
						<li><a
							href="${pageContext.request.contextPath}/orderBack/findPageBeanList.action?pageIndex=${pageBean.pageIndex-1}"
							aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
						</a></li>
					</c:if>
					...
					<c:forEach begin="1" end="${pageBean.totalPage}" var="page">
						<c:if test="${pageBean.pageIndex!=page}">
							<li><a
								href="${pageContext.request.contextPath}/orderBack/findPageBeanList.action?pageIndex=${page}">${page}</a></li>
						</c:if>
